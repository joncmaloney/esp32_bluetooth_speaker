...
int cnt=0,tmp=0;
extern const uint8_t src_audio_file_start[] asm("_binary_tst_wav_start");
extern const uint8_t src_audio_file_end[]   asm("_binary_tst_wav_end");
...
/* A2DP state machine handler for each state */
...
///function to convert name of BT to String
...
const unsigned int len_audio = src_audio_file_end - src_audio_file_start;

ESP_LOGI(BT_AV_TAG,"\n\n\n******************************************************************\n\n\n");
ESP_LOGI(BT_AV_TAG,"SNONO SAYS LEN OF AUDIO IS %d\n\n\n",len_audio);
...
//get name function of BT device
...
//        if (strcmp((char *)peer_bdname, "ESP_SPEAKER") != 0) {
       /* if (strcmp((char *)peer_bdname, "JBL Flip 2") != 0) {  
          return;
        }*/
...
////call back function for data stream
static int32_t bt_app_a2d_data_cb(uint8_t *data, int32_t len)
{
    if (len < 0 || data == NULL) {
        return 0;
        ...
        if(tmp<900)
{
    // generate random sequence
        for (int i = 0; i < len; i++) 
        {
         if ( (cnt+i+44) == (src_audio_file_end - src_audio_file_start))
        {
          tmp++;
         cnt =0; 
        }
        
         data[i] = src_audio_file_start[cnt+i+44];
        ...
        //len = src_audio_file_end - src_audio_file_start;
         
       /* for (int i = 0; i < (len >> 1); i++) {
        data[(i << 1)] = src_audio_file_start[(i << 1)] ;
        data[(i << 1) +1] = src_audio_file_start[(i << 1)+1];
